...
func Modules() ([]Module, error) {
	order := []string{"prompt", "aliases", "integrations", "completions-special", "interactive", "guards"}
	var mods []Module
...
if [ -d %[2]q ]; then
  for _of_m in %[2]q/prompt.zsh %[2]q/aliases.zsh %[2]q/integrations.zsh %[2]q/completions-special.zsh %[2]q/interactive.zsh %[2]q/guards.zsh; do
